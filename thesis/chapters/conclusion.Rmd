...
Third, I explored how the network structure can be harnessed to design effective management interventions. 
...
In \autoref{pollen-competition} I found that the number of shared pollinators, which is generally correlated to the number of partners a species has had a surprisingly low influence in the quality of pollination. 
...
Taken together, the results from \autoref{sdm-networks} and \autoref{pollen-competition} suggest that the point at which the trade-offs of sharing pollinators become too large might not only be species-dependent but might also be influenced by the environment. 
Specifically, the stress the environment induces in a species. 
In other words, it is possible that the differences between the "strategy" with which a species approaches the competition for pollination across different communities (which we studied in \autoref{pollen-competition}) could be partially explained by environmental differences between communities.
...
Additionally, increased environmental stress might also increase the risk of extinction of specialists that are not able not flexible enough to adapt. 
...
If climate change is able to modify the structure of pollination networks, we could expect dramatic changes on species control capacity and the network manageability. 
...
More data about the species interactions and their traits, and also more work like the one I presented in \autoref{structural-controllability} will hopefully solve one of the main challenges of network tools, and ecology as a whole: applying the general results of theoretical and computational ecology to the needs of conservation management on the ground. 
