...
However, with few exceptions [@tur_evaluating_2016; @aizen_reproductive_2010], most of what we know about pollen transfer and its relationship with key ecological factors are based on studies with two plant species, primarly from experimental settings where the rest of the community has been removed.
...
This evidence aligns with the notion that a species that interacts with few species does so strongly with each of them whereas a species that interacts with a large number of species does so comparatively weakly, a concept called asymmetry in pp networks [@thebault_does_2008; @bascompte_asymmetric_2006; @vazquez_species_2007].
...
Although the ecological factors were positively correlated (Figure \hyperlink{sup-pc}{S2}), the collinearity between predictors did not qualitatively affect our findings (Figure \hyperlink{sup-pc}{S3}).
